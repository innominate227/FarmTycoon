...
{
    public class WorkerPieMenuWindow : PieMenuWindow
    {
        public WorkerPieMenuWindow(Worker worker, Point centerPoint)            
        {
            base.Init(new string[] { "View", "Inventory", "Stats", "Change Name" }, centerPoint);
            ...
            base.ToolClicked += new Action<string, Point>(delegate(string tool, Point toolLoc)
            {
                ...
                if (tool == "View")
                {
                    poppedUpWindow = new WorkerViewWindow(worker);
                }
                else if (tool == "Inventory")
                {
                    poppedUpWindow = new InventoryWindow(worker.Inventory, false);
                    (poppedUpWindow as InventoryWindow).SetWindowName(worker);
                }
                else if (tool == "Stats")
                {
                    poppedUpWindow = new SingleTraitsWindow(worker.Traits, worker.Name);
                }
                else if (tool == "Change Name")
                {
                    poppedUpWindow = new ChangeNameWindow(worker);   
                }
